...
const App: React.FC = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // 제목 추가
      pdf.setFontSize(20);
      pdf.text(project.style + ' 스토리보드', pageWidth / 2, 20, { align: 'center' });
      
      pdf.setFontSize(10);
      pdf.text('Generated by Visionary Storyboard AI', pageWidth / 2, 28, { align: 'center' });
      ...
      for (let i = 0; i < project.panels.length; i++) {
        ...
        pdf.text(`Scene #${panel.sceneNumber}`, margin, yPosition);
        yPosition += 6;
        
        // 샷 타입
        pdf.setFontSize(9);
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Shot: ${panel.shotType}`, margin, yPosition);
        yPosition += 5;
        ...
        if (panel.imageUrl) {
          try {
            ...
            const imgHeight = imgWidth * 0.5625; // 16:9 비율
            
            pdf.addImage(panel.imageUrl, 'JPEG', margin, yPosition, imgWidth, imgHeight);
        ...
        // 설명
        pdf.setFontSize(10);
        const descLines = pdf.splitTextToSize(panel.description, pageWidth - (margin * 2));
        ...
        if (panel.dialogue) {
          ...
          pdf.setFontSize(9);
          const dialogueLines = pdf.splitTextToSize(`"${panel.dialogue}"`, pageWidth - (margin * 2));
          pdf.text(dialogueLines, margin, yPosition);
          yPosition += dialogueLines.length * 4 + 5;
        }
